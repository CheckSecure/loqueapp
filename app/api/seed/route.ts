...
// Profile IDs from the profiles table (confirmed from batch_suggestions logs)
...
export async function GET() {
  ...
  // Resolve Alexandra's profile id (may differ from auth UUID)
  ...
  for (const def of conversationDefs) {
    ...
    // Add participants (use user.id for Alexandra in conversation_participants
    // since messages page queries by user.id)
    ...
    // Insert messages with realistic timestamps spread over the past week
  ...
  for (const def of meetingDefs) {
    ...
    results[`Meeting: ${def.label}`] = mtgErr ? { error: mtgErr.message } : { ok: true, title: def.row.title }
  }
