...
- **Safe user deactivation system shipped (April 28).** Admin can now mark a user `account_status='deactivated'` instead of hard-deleting them, preserving matches, conversations, meeting history, audit trail, and reversibility. Schema additions: three audit columns on `profiles` (`deactivated_at timestamptz`, `deactivated_by uuid REFERENCES auth.users(id)`, `deactivation_reason text`). Existing CHECK constraint already permitted `'deactivated'` so no constraint change. Behavior: deactivation cancels future meetings (`status='cancelled'` on rows where the user is requester or recipient AND status IN `confirmed/requested/scheduled` AND scheduled_at>now()) silently — no notification fires because `meeting_declined` is not a real notification type in `lib/notifications/index.ts` despite being listed in the docs. Reactivation flips status only and clears the three audit columns; does NOT restore meetings, intros, blocks, or any other state. Backend selection sites that filter active-only: `lib/generate-recommendations.ts:802` (already), `app/api/admin/batch/[batchId]/generate-replacements/route.ts:240` (already), `lib/opportunities/eligibility.ts:52` (already), `lib/opportunities/matching.ts:417,472,566` (already). Four new active-only sites added: `app/actions.ts` (`submitIntroRequest`, commit `dee0566`); `app/api/intro-requests/express-interest/route.ts` STEP 0 guard before credit deduction (commit `47df6d5`, returns 410 with explicit "no credit was used" message); `app/api/admin/admin-create-match/route.ts` single `.in()` check before all other safety checks (commit `11ae8a7`, returns 409 with name in error); `lib/opportunities/connect.ts` after the blocked_users guard before match insert (commit `e2ba670`, adds `'user_inactive'` to ConnectFailureCode union). New admin endpoints `app/api/admin/users/deactivate/route.ts` and `app/api/admin/users/reactivate/route.ts` (commit `080fffd`); deactivate cancels meetings BEFORE flipping profile so partial failure leaves the profile untouched and admin can retry cleanly, both routes 404 on missing userId via `.select('id')`. Admin UI: inline Deactivate (red) / Reactivate (gold) buttons per row in `components/AdminMembersClient.tsx` with reason-required modal, finally-guarded loading state, inline error rendering (no `alert()`), amber pill for deactivated status (commit `f7cd906`). Read-side filter on `/dashboard/introductions` hides intro_requests rows where the other party has `account_status != 'active'` via single sidecar profiles query collecting all other-party IDs across admin intros + onboarding suggestions + batch suggestions, filtering with a TypeScript Set before render — covers both `deactivated` and `flagged` (commit `6a1d0b9`). Verified end-to-end on demo account `priya.rajan@google.com`: deactivation populated all four audit fields, hid her from Sarah's introductions page, blocked express-interest with credit preserved (Sarah stayed at 14 free credits across multiple attempts); reactivation restored her visibility; all preexisting matches stayed `active` throughout. Hard delete remains SQL-only per V1 lock; cleanup order documented in trap #21.
...
- **Replacement card generation when a slot is hidden by deactivation.** Currently when a user is deactivated, their cards disappear from the viewer's introductions page (commit `6a1d0b9`) but are not replaced. Viewers below their tier's recommendation count see fewer cards until the next cron refresh naturally tops up. Probably best resolved as part of the Phase 4 tier-aware top-up cron work rather than as its own logic — a single tier-cap-aware cron handles deactivation hides, user passes, and natural attrition uniformly.
- **"Former member" rendering on Network, Messages, and conversation surfaces.** When a user is deactivated, existing matches and conversations still display their original name, title, and company. Documented as V1 gap. Disabling the message composer for conversations involving a deactivated user requires piping participant status into `ConversationView` (currently it only fetches messages by conversation ID) — non-trivial, deferred.
- **`createNotificationSafe` recipient guard.** Cron-driven nudges (`nudge_interest`, `nudge_reply`, `low_credits`, `no_credits`, `new_batch`) may still fire to deactivated users. Harmless since they cannot log in, but visible as DB clutter. Add a single deactivated-recipient short-circuit at the top of `createNotificationSafe` when this becomes annoying.
- **Pending `intro_requests` and `batch_suggestions` archival on deactivation.** V1 left these rows untouched per the locked scope. Action endpoints reject any forward action so they're inert, but stale rows persist. If/when needed, a cleanup cron can flip status to `archived`. Do NOT do this synchronously in the deactivate handler — atomicity concerns and the V1 reactivation contract assumes a pure status flip.
- **Future-meeting cancellation does not notify the other party.** `meeting_declined` notification type is in the docs but not in the actual `NotificationType` union in `lib/notifications/index.ts`. Adding it requires the standard 5-step (union + COPY + LINK_BY_TYPE + fire from deactivate handler + bell display verification). Other party sees `status='cancelled'` on their meetings page next visit but gets no email/bell.
- **Admin UI does not prevent self-deactivation.** The Deactivate button renders on `bizdev91@gmail.com`'s own row in `/dashboard/admin/members`. Admin could click and lock themselves out. Cheap fix: hide the button when `user.id === currentAdminId` in `AdminMembersClient.tsx`.
- **Hard-delete UI for test accounts.** V1 deferred per scope. SQL-only via the trap #21 cleanup order. Build a separate gated tool when test-account churn justifies it. Two-step gate (deactivated-status + typed-email confirmation) per the original plan.
...
24. **Pasting TypeScript directly into the zsh prompt looks like a syntax error but doesn't update the file — and the next `npm run build` still passes because it's compiling unchanged code.** Workflow trap when working without an editor (no VS Code/Cursor open). The shell tries to execute the curly braces and fails with `parse error near '}'`. The actual file is unchanged, but a successful build creates false confidence that the edit "worked." Always confirm `git status` shows the file as modified before trusting a build result. Pair edits with `git diff <file>` to see what actually changed before committing.
25. **Claude Code can show a unified diff in chat and ask "Proceed?" without that being a real Edit tool prompt.** Approving such a chat-level prompt does NOT apply the edit — it's just a conversational question. The bug only surfaces later when manual smoke-testing reveals the change never landed. Always require the explicit `Edit file` tool prompt with the per-edit `1. Yes / 2. ... / 3. No` selection before approving. A "Proceed?" line in chat is a summary, not an edit.
