...
import { Sparkles, Loader2, CheckCircle, Users, AlertCircle } from 'lucide-react'
...
export default function AdminMutualInterest({ pairs }: { pairs: MutualPair[] }) {
  ...
  const [facilitated, setFacilitated] = useState<Record<string, boolean>>({})
  ...
  const handleFacilitate = async (pair: MutualPair) => {
    ...
    try {
      const res = await fetch('/api/admin/facilitate-intro', {
        ...
        body: JSON.stringify({ requestId: pair.request_a_id }),
      })
      const data = await res.json()
      if (data.success) {
        setFacilitated(prev => ({ ...prev, [key]: true }))
        router.refresh()
      } else {
        setErrors(prev => ({ ...prev, [key]: data.error || 'Failed to facilitate' }))
      }
    } catch (err) {
  ...
  return (
    <section className="mb-12">
      ...
      <div className="space-y-3">
        {pairs.map(pair => {
          const key = `${pair.user_a_id}-${pair.user_b_id}`
          const done = facilitated[key]
          const busy = facilitating === key
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                {done ? (
                  ...
                  >
                    ...
                    {busy ? 'Facilitating…' : 'Facilitate intro'}
                  </button>
                )}
              </div>
              {err && (
