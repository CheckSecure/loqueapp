    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={function(e){ if (e.target === e.currentTarget) onClose() }} role="dialog" aria-modal="true">
      <div ref={dialogRef} className="bg-white rounded-2xl shadow-xl max-w-xl w-full max-h-[92vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-5 space-y-6">
          ...
          {(seniority || expertise.length > 0) && (<section><h3 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Expertise</h3>{seniority && (<p className="text-sm text-slate-700 mb-2">{seniority}</p>)}{expertise.length > 0 && (<div className="flex flex-wrap gap-1.5">{expertise.slice(0, 12).map(function(tag){ return (<span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-slate-50 text-slate-600 border border-slate-100">{tag}</span>) })}</div>)}</section>)}
          {interests.length > 0 && (<section><h3 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Interests</h3><div className="flex flex-wrap gap-1.5">{interests.map(function(tag){ return (<span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-slate-50 text-slate-600 border border-slate-100">{tag}</span>) })}</div></section>)}
