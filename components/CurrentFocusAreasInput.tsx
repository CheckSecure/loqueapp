    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 px-3.5 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1B2850] focus:border-transparent transition disabled:bg-slate-50 disabled:text-slate-400"
        />
        <button
          type="button"
          onClick={() => add(draft)}
          disabled={!draft.trim() || atMax}
          className="inline-flex items-center gap-1 px-3 py-2 text-sm font-semibold text-brand-navy border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40"
        >
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {remainingSuggestions.map((s) => (
            <button
              ...
              type="button"
              onClick={() => add(s)}
              className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[13px] text-slate-600 hover:border-brand-gold/40 hover:text-brand-navy transition-colors"
            >
              + {s}
            </button>
          ))}
